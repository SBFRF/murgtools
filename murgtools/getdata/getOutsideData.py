...
def getSatelliteImagery(corners, filename=None, collection='sentinel-2-l2a',
    ...
    try:
        ...
        # STAC bbox is not reliable for pixel coordinate calculation when tags are present
        try:
            with tifffile.TiffFile(tmp_path) as tif:
                tags = tif.pages[0].tags
                # Tags 33922 (ModelTiepointTag) and 33550 (ModelPixelScaleTag)
                # may be missing for some GeoTIFFs; handle this gracefully.
                tiepoint_tag = tags[33922]  # may raise KeyError if missing
                scale_tag = tags[33550]     # may raise KeyError if missing
                tiepoint = tiepoint_tag.value  # (i, j, k, x, y, z) - UTM coordinates
                scale = scale_tag.value       # (scaleX, scaleY, scaleZ)
